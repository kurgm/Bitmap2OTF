...
class Bitmap(object):
	...
	def _makebold_x(self, boldtype=0, align=False):
		self.bitmap = [_makebold_row[boldtype](row, align) for row in self.bitmap]
	...
	def rotate(self, n=1):
		...
		if n == 2:
			...
			for r in self.bitmap:
				r.reverse()
			self.origin = (self.width - self.origin[0], self.height - self.origin[1])
			return
	...
	def toPolygons(self):
		...
		for y in range(self.height + 1):
			for x in range(self.width + 1):
				...
				vertices.extend([[x - self.origin[0], y - self.origin[1], dir_in, dir_out] for dir_in, dir_out in edgedirs])
		...
		while vertices:
			...
			while True:
				...
				next_dir_in = 3 - v[3]
				if   next_dir_in == _D:
					v1 = sorted([vn for vn in vertices_by_dir_in[next_dir_in] if vn[0] == v[0] and vn[1] > v[1]], key=lambda vn: vn[1] - v[1])[0]
				elif next_dir_in == _U:
					v1 = sorted([vn for vn in vertices_by_dir_in[next_dir_in] if vn[0] == v[0] and vn[1] < v[1]], key=lambda vn: v[1] - vn[1])[0]
				elif next_dir_in == _L:
					v1 = sorted([vn for vn in vertices_by_dir_in[next_dir_in] if vn[1] == v[1] and vn[0] > v[0]], key=lambda vn: vn[0] - v[0])[0]
				elif next_dir_in == _R:
					v1 = sorted([vn for vn in vertices_by_dir_in[next_dir_in] if vn[1] == v[1] and vn[0] < v[0]], key=lambda vn: v[0] - vn[0])[0]
				vertices.remove(v1)

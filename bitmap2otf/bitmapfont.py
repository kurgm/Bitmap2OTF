...
class BitmapFont(object):
	...
	def appendGlyph(self, glyph):
		if self.getGlyphByName(glyph.name) is not None:
			if glyph.name != "uni0020":
				print("Info: there is already a glyph with name '{}' in the font and the new glyph was not added.".format(glyph.name))
			return
		if glyph.codepoint != -1 and self.getGlyphByCodepoint(glyph.codepoint) is not None:
			print("Info: there is already a glyph with codepoint U+{:04x} in the font and the new glyph was not added.".format(glyph.codepoint))
			return
	...
	def getXAvgCharWidth(self, dw=100.0):
		...
		aws = [g.bitmap.advanceWidth for g in self.glyphs if 0x61 <= g.codepoint <= 0x7A or g.codepoint == 0x20] or [0]
	...
	def getOS2ulUnicodeRanges(self):
		...
		for g in self.glyphs:
			...
			if g.codepoint > 0xFFFF:
				ranges[57] = True  # Non-Plane 0

			for r0, r1, bit in unicodeRanges:
				if r0 <= g.codepoint <= r1:
					ranges[bit] = True
					break
	...
	def isFixedPitch(self):
		nonzeroWidths = [g.bitmap.advanceWidth for g in self.glyphs if g.bitmap.advanceWidth != 0] or [0]
		w0 = nonzeroWidths[0]

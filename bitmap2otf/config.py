...
def _findChildOrError(elem, path, attribute=None):
	c = elem.find("./" + path)
	if c is None:
		raise ConfigFileError("element '{}' not found under element '{}'".format(path, elem.tag))
...
def _findAttrOrError(elem, attribute):
	...
	if v is None:
		raise ConfigFileError("element '{}' has no attribute '{}'".format(elem.tag, attribute))
	return v
...
def _getGlyphOptions(elem, default=[], basepath=""):
	...
	for key, val in items:
		...
		if key in ("x", "y") and elem.tag in ("Image", "Glyph", "Glyphs"):
			...
			raise ConfigFileError("element '{}' has unknown glyph attribute '{}'".format(elem.tag, key))
		res.append((key, val))
	return res
...
def _myord_filterTabAndNewlines(s):
	return [c for c in _myord(s) if c not in (9, 10, 13) and not _isVariationSelector(c)]  # \t, \r, \n
...
def _getNameOrCodepointList(elem, allowAllGlyphs=False):
	...
	for subelem in elem:
		...
		if subelem.tag != "Glyph":
			raise ConfigFileError("unknown element '{}' found in element '{}'".format(subelem.tag, elem.tag))
		if subelem.get("name") is not None:
			...
			print("Warning: ignoring Glyph element which has none of attributes 'name', 'codepoint' and 'char'.")
		res.extend(("codepoint", c) for c in _myord_filterTabAndNewlines(subelem.tail))
	return res
...
class FontInfo(object):
	...
	def fromXML(self, elem):
		if elem.tag == "setting":
			...
			if key in ("ascent", "descent", "x-height", "vertascent", "vertdescent", "bold", "italic"):
				...
				print("Warning: ignoring unknown parameter '{}' in FontInfo.".format(key))
				return
			...
			if not (len(platformID) == len(platEncID) == len(langID)):
				raise ConfigFileError("each of platformID, platEncID and langID must have the same number of values")
			...
			for r in elem:
				if r.get("nameID") is not None:
					...
					if nameID is None:
						print("Warning: ignoring unknown keyword '{}' in namerecord.".format(keyword))
						continue
				records[nameID] = _findAttrOrError(r, "value")

			self.names.append(NameRecord(platformID, platEncID, langID, records))
	...
	def getCFFNames(self):
		...
		for namerecord in self.names:
			...
			raise ConfigFileError("namerecords to use as CFF names were not found")

		if _nameKeyword2nameID["postscriptname"] not in self.cffNames:
			raise ConfigFileError("'postscriptName' record was not found in the namerecords to use as CFF names")
...
class Config(object):
	def __init__(self, configfilepath):
		...
		self.templateTTXpath = os.path.join(configdirpath, _findChildOrError(root, "TemplateTTX", "src"))

		self.outputTo = os.path.splitext(configfilepath)[0] + ".otf"
		outputTo = root.find("./Output")
		if outputTo is not None:
			self.outputTo = os.path.join(configdirpath, outputTo.get("path", self.outputTo))
		...
		if dotshape is not None:
			...
			if shapetype is not None:
				if shapetype != "pixel-outline":
					raise ConfigFileError("unknown dot shape type '{}'".format(shapetype))
				self.outlineCfg["shapetype"] = shapetype
			else:
				shapesrc = dotshape.get("src")
				if shapesrc is None:
					raise ConfigFileError("Either attribute 'type' or 'src' is required for 'DotShape' element")
				self.outlineCfg["shapetype"] = None
				self.outlineCfg["shapesrc"] = os.path.join(configdirpath, shapesrc)
				self.outlineCfg["shapescale_x"] = float(dotshape.get("width", "1.0"))
				self.outlineCfg["shapescale_y"] = float(dotshape.get("height", "1.0"))
		...
		for glyphsrc in glyphsrcs:
			glyphsrc_settings = _getGlyphOptions(glyphsrc, default_settings, basepath=configdirpath)
			if glyphsrc.tag == "BitmapData":
				glyph = GlyphSourceBitmap(glyphsrc.text, **dict(glyphsrc_settings))
				...
				for char in chars:
					glyph = GlyphSourceSpace(**dict(glyphsrc_settings + [("codepoint", char)]))
					self.glyphsources.append(glyph)
				...
				for glypharea in glyphsrc:
					glypharea_settings = _getGlyphOptions(glypharea, glyphsrc_settings, basepath=configdirpath)
					if glypharea.tag == "Glyph":
						...
						charsperrow = safeEval(glypharea.get("charsPerRow", image_charsperrow))
						if stepx is None or stepy is None or charsperrow is None:
							raise ConfigFileError("all of stepx, stepy and charsPerRow attributes must be specified in either Image or Glyphs element")
						...
						raise ConfigFileError("unknown element '{}' in Image element".format(glypharea.tag))
			elif glyphsrc.tag == "Copy":
				if glyphsrc.get("fromChar") is not None:
					glyphopt = ("srccodepoint", _myord(glyphsrc.get("fromChar"))[0])
				elif glyphsrc.get("fromCodepoint") is not None:
					glyphopt = ("srccodepoint", safeEval(glyphsrc.get("fromCodepoint")))
				elif glyphsrc.get("fromName") is not None:
					glyphopt = ("srcname", glyphsrc.get("fromName"))
				else:
					raise ConfigFileError("fromChar, fromCodepoint or fromName attribute is required for 'Copy' element")
				glyph = GlyphSourceGlyph(**dict(glyphsrc_settings + [glyphopt]))
				self.glyphsources.append(glyph)
			else:
				print("Warning: ignoring unknown glyph source '{}'.".format(glyphsrc.tag))
		self.effects = []
		effects = root.find("./Effects")
		if effects is not None:
			for effect in effects:
				glyphopts = _getNameOrCodepointList(effect, allowAllGlyphs=True)
		...
		self.templateTTX2 = [os.path.join(configdirpath, _findAttrOrError(templateTTX2, "src")) for templateTTX2 in root.iterfind("./TemplateTTX2")]

	def toBitmapFont(self):
		bitmapfont = BitmapFont(fontinfo=self.fontinfo, outlineCfg=self.outlineCfg, generateBitmap=self.generateBitmap, glyphs=[])
		...
		for gopts, effname, effargs in self.effects:
			for goptname, val in gopts:
				if goptname == "codepoint":
					...
					if glyphs[0] is None:
						print("Warning: glyph to apply effect '{}' (U+{:04x}) was not found.".format(effname, val))
						continue
				elif goptname == "name":
					glyphs = [bitmapfont.getGlyphByName(val)]
					if glyphs[0] is None:
						print("Warning: glyph to apply effect '{}' (name='{}') was not found.".format(effname, val))
						continue
	...
	def shape(self):
		if self.outlineCfg["shapetype"] is not None:
			...
			shape = DotShapeExternal(self.outlineCfg["shapesrc"], self.outlineCfg["shapescale_x"], self.outlineCfg["shapescale_y"])
...
class GlyphSourceBitmap(GlyphSource):
	def __init__(self, bitmapstring, bitmapwidth, bitmapheight, *args, **kwargs):
		super(GlyphSourceBitmap, self).__init__(bitmapwidth, bitmapheight, *args, **kwargs)
		bitmapstring = _GBITMAP_IGNORE_REGEXP.sub("", bitmapstring)
		bitmap = [c not in "0 ." for c in bitmapstring if not 0xDC00 <= ord(c) <= 0xDFFF]
		self.bitmap = [bitmap[i:i + bitmapwidth] for i in range(0, bitmapwidth * bitmapheight, bitmapwidth)]
		self.bitmap.reverse()
...
class GlyphSourceImage(GlyphSource):
	...
	def _toGlyph(self, font):
		...
		imagedata = list(img.crop((self.x, self.y, self.x + w, self.y + h)).getdata())
		bitmap = [[v == 0 for v in imagedata[i:i + w]] for i in range(0, w * h, w)]
		bitmap.reverse()
